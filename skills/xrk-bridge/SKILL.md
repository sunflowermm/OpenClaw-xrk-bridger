...
- `base64://...`：直接使用。
- `http://` / `https://`：直接使用。
- 其他：当作本机路径（支持 `file://` 前缀），XRK-AGT 会读取并转成 `base64://...`。
- 按内容或扩展名识别类型后，会以对应 QQ 段发送：
  - **图片** → 图片段
  - **视频**（mp4、webm、mov 等）→ 视频段
  - **音频**（mp3、wav、m4a 等）→ 语音段
  - **办公/其他**（ppt、doc、pdf 等）→ 文件段，文件名会规范为正确后缀（如 cfb→ppt）。

**你应该怎么做：**

- 只填真实存在的本机路径或可访问的 URL；`files` 中尽量带 `name` 以便 QQ 显示正确文件名。
- 不确定文件是否存在时：优先只回 `text`，或让用户提供路径。

**你绝对不要做：**

- 不要凭空编造路径。
- 不要把超大文件直接塞进 `base64://`（会拖垮链路/模型输出）。
